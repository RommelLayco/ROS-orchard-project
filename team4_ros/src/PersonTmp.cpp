...
void updateCurrentVelocity() {
    ...
    {    
        currentAngle = 3.14 + currentAngle;
        desiredAngle = 3.14 + desiredAngle;
        double diffAngle = currentAngle - desiredAngle;
       

        ROS_INFO("currentAngle is : %f",currentAngle); 
        ROS_INFO("desiredAngle is : %f",desiredAngle); 
        ROS_INFO("diffAngle is : %f",diffAngle); 
        ...
        if (currentAngle - desiredAngle > 0.1 || desiredAngle - currentAngle > 0.1)
        {
            ...
            if (diffAngle < 0)
            {
                ...
                currentVelocity.linear.x = 0;
                currentVelocity.angular.z = -1.5;
                ROS_INFO("Spinning clockwise :   %f", currentAngle);
            }
            ...
            else if(diffAngle > 0)
            {
                ...
                currentVelocity.linear.x = 0;
                currentVelocity.angular.z = 1.5;
                ROS_INFO("Spinning anticlockwise : %f", currentAngle);
            }


      /*  // If the deifference between current angle and desired angle is less than 1 stop spining
        if (currentAngle - desiredAngle > 1 || desiredAngle - currentAngle > 1)
        {
            // Spin
            currentVelocity.linear.x = 0;
            currentVelocity.angular.z = 1.5;
