...
float desiredAngles[4] = {-1.57, -3.14, 1.57, 0.00 };
void generateDesiredLocations(){
  	const int ROWS = 4;
	const int COLS = 2;
	const int BUFFSIZE = 80;
	 
	//Read value of positions from tractorLocations
	float positions[ROWS][COLS];
	char buff[BUFFSIZE]; // a buffer to temporarily park the data
	std::ifstream infile("src/se306-1/locations/tractorLocations");
	std::stringstream ss;
    ...
    desiredLocation1.x = positions[1][0];
    desiredLocation1.y = positions[1][1];
    desiredLocation1.z = 0;
	ROS_INFO("DesiredLocation1: %f,%f", positions[1][0],positions[1][1]);
	ROS_INFO("DesiredLocation2: %f,%f", positions[2][0],positions[2][1]);
	ROS_INFO("DesiredLocation3: %f,%f", positions[3][0],positions[3][1]);
	ROS_INFO("DesiredLocation4: %f,%f", positions[0][0],positions[0][1]);

    geometry_msgs::Point desiredLocation2;
    desiredLocation2.x = positions[2][0];
    desiredLocation2.y = positions[2][1];;
    desiredLocation2.z = 0;

	geometry_msgs::Point desiredLocation3;
    desiredLocation3.x = positions[3][0];;
    desiredLocation3.y = positions[3][1];;
    desiredLocation3.z = 0;

	geometry_msgs::Point desiredLocation4;
    desiredLocation4.x = positions[0][0];
    desiredLocation4.y = positions[0][1];
...
//Rotate to desired angle and at angular speed as specified
